...
def seed_data_platform_project(session: Session) -> int:
    ...
    tasks = [
        ...
        TaskRow(
            task_id=100, project_id=pid,
            description="Audit and catalog existing ETL pipelines",
            owner_name="Priya", owner_email="priya@test.com",
            due_date=_d("2026-02-21"), status="complete", priority="high",
            phase_id=100, depends_on="[]",
        ),
        TaskRow(
            task_id=101, project_id=pid,
            description="Design lakehouse target architecture",
            owner_name="James", owner_email="james@test.com",
            due_date=_d("2026-02-28"), status="complete", priority="high",
            phase_id=100, depends_on=json.dumps([100]),
        ),
        TaskRow(
            task_id=102, project_id=pid,
            description="Provision cloud infrastructure (Databricks, S3, networking)",
            owner_name="James", owner_email="james@test.com",
            due_date=_d("2026-03-07"), status="complete", priority="high",
            phase_id=100, depends_on=json.dumps([101]),
        ),
        # Phase 2 — Migration (in progress)
        TaskRow(
            task_id=103, project_id=pid,
            description="Migrate batch reporting pipelines (priority 1-10)",
            owner_name="Priya", owner_email="priya@test.com",
            due_date=_d("2026-03-28"), status="in_progress", priority="high",
            phase_id=101, depends_on=json.dumps([102]),
        ),
        TaskRow(
            task_id=104, project_id=pid,
            description="Migrate batch reporting pipelines (priority 11-20)",
            owner_name="Lena", owner_email="lena@test.com",
            due_date=_d("2026-04-11"), status="not_started", priority="medium",
            phase_id=101, depends_on=json.dumps([103]),
        ),
        TaskRow(
            task_id=105, project_id=pid,
            description="Build real-time streaming ingestion layer",
            owner_name="James", owner_email="james@test.com",
            due_date=_d("2026-04-18"), status="in_progress", priority="high",
            phase_id=101, depends_on=json.dumps([102]),
        ),
        # Phase 3 — Validation & Cutover (not started)
        TaskRow(
            task_id=106, project_id=pid,
            description="Run parallel data quality validation (old vs new)",
            owner_name="Priya", owner_email="priya@test.com",
            due_date=_d("2026-05-02"), status="not_started", priority="high",
            phase_id=102, depends_on=json.dumps([104, 105]),
        ),
        TaskRow(
            task_id=107, project_id=pid,
            description="SOC 2 compliance audit for new platform",
            owner_name="Lena", owner_email="lena@test.com",
            due_date=_d("2026-05-09"), status="not_started", priority="high",
            phase_id=102, depends_on=json.dumps([106]),
            external_dependency="External auditor — engagement confirmed for w/c 2026-05-04.",
        ),
        TaskRow(
            task_id=108, project_id=pid,
            description="Production cutover and legacy decommission",
            owner_name="James", owner_email="james@test.com",
            due_date=_d("2026-05-15"), status="not_started", priority="high",
            phase_id=102, depends_on=json.dumps([106, 107]),
        ),
    ]
    session.add_all(tasks)
    ...
    raid_items = [
        RaidItemRow(
            raid_id=100, project_id=pid, type="risk",
            ...
            owner="James", raised_date=_d("2026-03-12"), status="open",
        ...
        RaidItemRow(
            raid_id=101, project_id=pid, type="assumption",
            ...
            owner="Priya", raised_date=_d("2026-02-20"), status="open",
        ...
        RaidItemRow(
            raid_id=102, project_id=pid, type="decision",
        ...
        RaidItemRow(
            ...
            owner="Priya", raised_date=_d("2026-03-18"), status="open",
    ...
    actions = [
        ActionRow(
            action_id=100, project_id=pid,
            description="Run streaming load test and document p99 latency results.",
            owner_name="James", owner_email="james@test.com",
            due_date=_d("2026-04-04"), status="open",
            source_raid_id=100,
        ),
        ActionRow(
            action_id=101, project_id=pid,
            description="Reverse-engineer undocumented pipeline dependencies and document them.",
            owner_name="Priya", owner_email="priya@test.com",
            due_date=_d("2026-03-25"), status="open",
            source_raid_id=103, source_task_id=103,
        ),
    ]
    session.add_all(actions)
    ...
    messages = [
        MessageRow(
            message_id=str(uuid.uuid4()), project_id=pid,
            direction="inbound", timestamp="2026-03-20T10:30:00",
            owner_name="Priya", owner_email="priya@test.com",
            message=(
            ...
            sender_name="Priya", sender_email="priya@test.com",
        ),
        MessageRow(
            message_id=str(uuid.uuid4()), project_id=pid,
            direction="inbound", timestamp="2026-03-20T11:15:00",
            owner_name="James", owner_email="james@test.com",
            message=(
            ...
            sender_name="James", sender_email="james@test.com",
        ),
    ]
    session.add_all(messages)
...
def seed_mobile_app_project(session: Session) -> int:
    ...
    phases = [
        PhaseRow(
            phase_id=200, project_id=pid,
            name="Design & Prototyping",
            description="User research, UI/UX design, interactive prototypes.",
            planned_start=_d("2026-03-01"), planned_end=_d("2026-03-21"),
        ),
        PhaseRow(
            phase_id=201, project_id=pid,
            name="Development",
            description="Core app development, API integration, biometric auth.",
            planned_start=_d("2026-03-24"), planned_end=_d("2026-05-02"),
        ),
        PhaseRow(
            phase_id=202, project_id=pid,
            name="Beta & Launch",
            description="Beta testing, app store submission, staged rollout.",
            planned_start=_d("2026-05-05"), planned_end=_d("2026-05-30"),
        ),
    ]
    session.add_all(phases)
    ...
    tasks = [
        ...
        TaskRow(
            task_id=200, project_id=pid,
            description="Conduct user research and define personas",
            owner_name="Anika", owner_email="anika@test.com",
            due_date=_d("2026-03-07"), status="complete", priority="high",
            phase_id=200, depends_on="[]",
        ),
        TaskRow(
            task_id=201, project_id=pid,
            description="Create high-fidelity UI mockups",
            owner_name="Anika", owner_email="anika@test.com",
            due_date=_d("2026-03-14"), status="complete", priority="high",
            phase_id=200, depends_on=json.dumps([200]),
        ),
        TaskRow(
            task_id=202, project_id=pid,
            description="Build interactive prototype and run usability tests",
            owner_name="Anika", owner_email="anika@test.com",
            due_date=_d("2026-03-21"), status="complete", priority="medium",
            phase_id=200, depends_on=json.dumps([201]),
        ),
        # Phase 2 — Development (in progress / blocked)
        TaskRow(
            task_id=203, project_id=pid,
            description="Develop core app shell and navigation (React Native)",
            owner_name="Tom", owner_email="tom@test.com",
            due_date=_d("2026-04-04"), status="in_progress", priority="high",
            phase_id=201, depends_on=json.dumps([202]),
        ),
        TaskRow(
            task_id=204, project_id=pid,
            description="Integrate biometric authentication SDK",
            owner_name="Tom", owner_email="tom@test.com",
            due_date=_d("2026-04-18"), status="blocked", priority="high",
            phase_id=201, depends_on=json.dumps([203]),
            blocked_reason=(
        ...
        TaskRow(
            task_id=205, project_id=pid,
            description="Build real-time transaction notification service",
            owner_name="Raj", owner_email="raj@test.com",
            due_date=_d("2026-04-18"), status="in_progress", priority="high",
            phase_id=201, depends_on=json.dumps([203]),
        ),
        TaskRow(
            task_id=206, project_id=pid,
        ...
        TaskRow(
            task_id=207, project_id=pid,
            description="Beta testing program (500 users)",
            owner_name="Anika", owner_email="anika@test.com",
            due_date=_d("2026-05-16"), status="not_started", priority="medium",
            phase_id=202, depends_on=json.dumps([204, 205, 206]),
        ),
        TaskRow(
            task_id=208, project_id=pid,
            description="App store submission and staged rollout",
            owner_name="Tom", owner_email="tom@test.com",
            due_date=_d("2026-05-30"), status="not_started", priority="high",
            phase_id=202, depends_on=json.dumps([207]),
        ),
    ]
    session.add_all(tasks)
    ...
    raid_items = [
        RaidItemRow(
            raid_id=200, project_id=pid, type="risk",
            ...
            owner="Tom", raised_date=_d("2026-03-18"), status="open",
        ...
        RaidItemRow(
            raid_id=201, project_id=pid, type="risk",
            ...
            owner="Tom", raised_date=_d("2026-03-10"), status="open",
        ...
        RaidItemRow(
            raid_id=202, project_id=pid, type="issue",
        ...
        RaidItemRow(
            raid_id=203, project_id=pid, type="assumption",
            ...
            owner="Raj", raised_date=_d("2026-03-05"), status="open",
        ...
        RaidItemRow(
            raid_id=204, project_id=pid, type="decision",
    ...
    actions = [
        ActionRow(
            action_id=200, project_id=pid,
            description="Escalate biometric SDK bug to vendor management and request fix ETA.",
            owner_name="Tom", owner_email="tom@test.com",
            due_date=_d("2026-03-20"), status="open",
            source_raid_id=200, source_task_id=204,
        ),
        ActionRow(
            action_id=201, project_id=pid,
    ...
    messages = [
        MessageRow(
            message_id=str(uuid.uuid4()), project_id=pid,
            direction="inbound", timestamp="2026-03-20T08:45:00",
            owner_name="Tom", owner_email="tom@test.com",
            message=(
            ...
            sender_name="Tom", sender_email="tom@test.com",
        ),
        MessageRow(
            message_id=str(uuid.uuid4()), project_id=pid,
            direction="inbound", timestamp="2026-03-20T09:20:00",
            owner_name="Raj", owner_email="raj@test.com",
            message=(
            ...
            sender_name="Raj", sender_email="raj@test.com",
        ),
    ]
    session.add_all(messages)
